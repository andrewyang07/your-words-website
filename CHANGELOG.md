...
#### 筆記本編輯器 UI 優化
- **固定格式工具欄** - 替換 FloatingMenu，在編輯器上方顯示固定工具欄（粗體、斜體、標題、引用、列表、有序列表、分割線），按鈕高亮顯示當前格式狀態
- **幫助說明改為模態框** - 將內嵌的幫助說明移至「幫助」按鈕觸發的模態彈窗，點擊遮罩或 X 按鈕關閉，保持編輯區域整潔
- **保留 BubbleMenu** - 選中文字時仍顯示浮動格式工具欄
