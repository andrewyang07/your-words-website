...
export default function Slider({ id, label, min, max, value, onChange, showValue = true, className = '' }: SliderProps) {
    return (
        <div className={`flex items-center gap-1.5 ${className}`}>
            {label && (
                <label htmlFor={id} className="font-chinese text-bible-700 dark:text-bible-300 whitespace-nowrap text-xs sm:text-sm flex-shrink-0">
                    {label}
                </label>
            )}
            <input
            ...
            />
            {showValue && (
                <span className="font-chinese text-bible-700 dark:text-bible-300 font-semibold min-w-[2ch] text-center text-xs sm:text-sm flex-shrink-0">{value}</span>
            )}
        </div>
    );
}
