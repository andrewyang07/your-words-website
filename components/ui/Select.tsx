...
export default function Select<T extends string>({ value, onChange, options, className = '', label }: SelectProps<T>) {
    ...
    return (
        <div className={`relative z-[220] overflow-visible ${className}`}>
            ...
            <Listbox value={value} onChange={onChange}>
                <div className="relative overflow-visible">
                    ...
                    <Transition as={Fragment} leave="transition ease-in duration-100" leaveFrom="opacity-100" leaveTo="opacity-0">
                        <Listbox.Options className="absolute z-[9999] mt-2 max-h-[min(24rem,60vh)] w-full overflow-auto rounded-2xl border border-stone-900/10 bg-white py-1 shadow-[0_24px_70px_rgba(68,64,60,0.24)] ring-1 ring-black/5 focus:outline-none dark:border-white/10 dark:bg-gray-950 scrollbar-thin">
                            {options.map((option) => (
                                <Listbox.Option
